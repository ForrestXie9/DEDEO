...
class RBFNmv_g(BaseEstimator):
    ...
    def kernel_(self, data_point):
        ...
        for i in range(n1):
            rdistMat[i, :] = np.sum(np.abs(xr1[i] - xr2)/(cmax - cmin), axis=1)       ##############################修改在这里######################################
        ...
        # a = np.exp(-0.5 * distMat / self.sigma ** 2)
        return np.exp(-0.5 * distMat / self.sigma ** 2)
        # return np.exp(-0.5 * distMat)
    ...
    def calsigma(self):
        ...
        for i in range(self.num_neurons - 1):
            for j in range(i + 1, self.num_neurons):
                # d = np.hstack([(self.centers[i, :self.r] - self.centers[j, :self.r])/(self.cxmax - self.cxmin), self.centers[i, self.r:] != self.centers[j, self.r:]])
                d = np.hstack([(self.centers[i, :self.r] - self.centers[j, :self.r]),
                               self.centers[i, self.r:] != self.centers[j, self.r:]])
    ...
    def kmeans(self, X, n_clusters):
        ...
        while (delta > 1e-4 and c < 500):
            c_f = centers.copy()
            x12 = np.concatenate((X[:, :self.r], centers[:, :self.r]), axis=0)
            cmax = np.max(x12, axis=0)
            cmin = np.min(x12, axis=0)
            # 划分簇
            for i in range(nums):

                dist_r = np.sum(np.abs(X[i, :self.r] - centers[:, :self.r])/(cmax - cmin),  ############################修改在这里######################################
                                axis=1)
